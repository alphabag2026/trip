...
const baggageStatusLabels: Record<string, { label: string; color: string; icon: string }> = {
  checked_in: { label: "체크인 완료", color: "bg-blue-500/20 text-blue-400", icon: "📦" },
  loaded: { label: "탑재 완료", color: "bg-cyan-500/20 text-cyan-400", icon: "✈️" },
  in_transit: { label: "운송 중", color: "bg-yellow-500/20 text-yellow-400", icon: "🚚" },
  arrived: { label: "도착", color: "bg-green-500/20 text-green-400", icon: "📍" },
  claimed: { label: "수령 완료", color: "bg-emerald-500/20 text-emerald-400", icon: "✅" },
  delayed: { label: "지연", color: "bg-orange-500/20 text-orange-400", icon: "⏳" },
  lost: { label: "분실", color: "bg-red-500/20 text-red-400", icon: "❌" },
};

const flightStatusLabels: Record<string, { label: string; color: string }> = {
  scheduled: { label: "예정", color: "bg-slate-500/20 text-slate-400" },
  boarding: { label: "탑승 중", color: "bg-blue-500/20 text-blue-400" },
  departed: { label: "출발", color: "bg-cyan-500/20 text-cyan-400" },
  in_air: { label: "비행 중", color: "bg-sky-500/20 text-sky-400" },
  landed: { label: "착륙", color: "bg-green-500/20 text-green-400" },
  delayed: { label: "지연", color: "bg-orange-500/20 text-orange-400" },
  cancelled: { label: "결항", color: "bg-red-500/20 text-red-400" },
};
...
const checkinStatusLabels: Record<string, { label: string; color: string }> = {
  ...
  online_checkin: { label: "온라인 체크인", color: "bg-blue-500/20 text-blue-400" },
  counter_checkin: { label: "카운터 체크인", color: "bg-cyan-500/20 text-cyan-400" },
  boarding_pass_issued: { label: "탑승권 발급", color: "bg-green-500/20 text-green-400" },
  boarded: { label: "탑승 완료", color: "bg-emerald-500/20 text-emerald-400" },
};
...
export default function FlightTracker() {
  ...
  const { data: lookupData, refetch: refetchLookup } = trpc.registration.lookup.useQuery(
    { name, phone },
  ...
  const createBaggage = trpc.baggage.create.useMutation({
    onSuccess: () => { refetchBaggage(); toast.success("수화물이 등록되었습니다."); },
  });
  ...
  const uploadTagPhoto = trpc.baggage.uploadTagPhoto.useMutation({
    onSuccess: (data) => {
      refetchBaggage();
      if (data.tagNumber) {
        toast.success(`태그 번호 인식: ${data.tagNumber}`);
      } else {
        toast.info("사진이 업로드되었습니다. 태그 번호 인식에 실패했습니다.");
      }
        ...
        <header className="border-b border-border/50 backdrop-blur-sm sticky top-0 z-50 bg-background/80">
          <div className="container flex items-center h-14">
            <Link href="/" className="flex items-center gap-2 text-muted-foreground hover:text-foreground">
              ...
              <span className="font-semibold">항공편 & 수화물 추적</span>
            </Link>
          </div>
        </header>
        <div className="container max-w-md py-12">
          <Card className="bg-card border-border">
            <CardHeader className="text-center">
              ...
              <CardTitle>항공편 & 수화물 추적</CardTitle>
              <p className="text-sm text-muted-foreground mt-1">
                신청 시 입력한 이름과 전화번호로 조회하세요
              </p>
            </CardHeader>
            <CardContent>
              <form onSubmit={handleSearch} className="space-y-4">
                ...
                <div>
                  ...
                  <Input id="phone" type="tel" value={phone} onChange={e => setPhone(e.target.value)} placeholder="010-1234-5678" required />
                </div>
      ...
      <header className="border-b border-border/50 backdrop-blur-sm sticky top-0 z-50 bg-background/80">
        <div className="container flex items-center justify-between h-14">
          <Link href="/" className="flex items-center gap-2 text-muted-foreground hover:text-foreground">
            <ArrowLeft className="h-5 w-5" />
            <Plane className="h-5 w-5 text-primary" />
            <span className="font-semibold">항공편 & 수화물</span>
          </Link>
      ...
      <div className="container max-w-2xl py-6">
        ...
        <div className="bg-primary/10 border border-primary/20 rounded-lg p-4 mb-6 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">{reg.phone} · {reg.locationType === "overseas" ? "해외" : "내륙"}</p>
          </div>
        </div>
        ...
        <Tabs defaultValue="flights" className="space-y-4">
          <TabsList className="grid w-full grid-cols-3">
            <TabsTrigger value="flights" className="text-xs">
              <Plane className="h-3.5 w-3.5 mr-1" />항공편
            </TabsTrigger>
            <TabsTrigger value="baggage" className="text-xs">
              <Luggage className="h-3.5 w-3.5 mr-1" />수화물
            </TabsTrigger>
            <TabsTrigger value="checkin" className="text-xs">
              <DoorOpen className="h-3.5 w-3.5 mr-1" />체크인
            </TabsTrigger>
          </TabsList>
            ...
            {(!flights || flights.length === 0) ? (
              <Card className="bg-card border-border">
                <CardContent className="py-12 text-center">
                  ...
                  <p className="text-muted-foreground">등록된 항공편 정보가 없습니다.</p>
                  <p className="text-xs text-muted-foreground mt-1">관리자가 항공편을 배정하면 여기에 표시됩니다.</p>
                </CardContent>
              </Card>
            ) : flights.map((f: any) => (
                  ...
                  <div className="flex items-center gap-4">
                    <div className="text-center flex-1">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {f.scheduledDeparture ? new Date(f.scheduledDeparture).toLocaleString("ko-KR", { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" }) : "-"}
                      </p>
                    </div>
                    ...
                    <div className="text-center flex-1">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {f.scheduledArrival ? new Date(f.scheduledArrival).toLocaleString("ko-KR", { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" }) : "-"}
                      </p>
                    </div>
                  </div>
                  {f.delayMinutes > 0 && (
                    <div className="bg-orange-500/10 border border-orange-500/20 rounded-lg p-3 flex items-center gap-2">
                      ...
                      <span className="text-sm text-orange-400">
                        {f.delayMinutes}분 지연{f.actualDeparture ? ` · 변경 출발: ${new Date(f.actualDeparture).toLocaleTimeString("ko-KR", { hour: "2-digit", minute: "2-digit" })}` : ""}
                      </span>
                    </div>
                  )}
            ...
            {(!baggage || baggage.length === 0) ? (
              <Card className="bg-card border-border">
                <CardContent className="py-12 text-center">
                  ...
                  <p className="text-muted-foreground">등록된 수화물이 없습니다.</p>
                  <p className="text-xs text-muted-foreground mt-1">수화물을 등록하고 태그 사진을 업로드하면 추적할 수 있습니다.</p>
                </CardContent>
              </Card>
            ) : baggage.map((b: any) => {
                ...
                <Card key={b.id} className="bg-card border-border">
                  <CardContent className="p-4 space-y-3">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <p className="font-semibold">
                            {b.tagNumber ? `#${b.tagNumber}` : `수화물 ${b.id}`}
                          </p>
                          <p className="text-xs text-muted-foreground">
                            {b.baggageType || "일반"} {b.weight ? `· ${b.weight}` : ""}
                          </p>
                        </div>
                      </div>
                    ...
                    <div className="flex justify-between text-[10px] text-muted-foreground">
                      <span>체크인</span><span>탑재</span><span>운송</span><span>도착</span><span>수령</span>
                    </div>

                    {/* 태그 사진 */}
                    {b.tagPhotoUrl ? (
                      <div className="relative">
                        <img src={b.tagPhotoUrl} alt="수화물 태그" className="w-full h-32 object-cover rounded-lg" />
                        {b.ocrResult && (
                          <div className="absolute bottom-2 left-2 right-2 bg-black/70 backdrop-blur-sm rounded-lg p-2 text-xs">
                            <div className="flex justify-between text-white">
                              <span>태그: {b.ocrResult.tagNumber}</span>
                              <span>{b.ocrResult.airline}</span>
                            </div>
                            <div className="flex justify-between text-white/70">
                              <span>목적지: {b.ocrResult.destination}</span>
                              <span>신뢰도: {b.ocrResult.confidence}</span>
                            </div>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="border-2 border-dashed border-border rounded-lg p-4 text-center">
                        ...
                        <p className="text-xs text-muted-foreground mb-2">수화물 태그 사진을 업로드하면 번호를 자동 인식합니다</p>
                        <input
                          type="file"
                          accept="image/*"
                          capture="environment"
                        ...
                        <label htmlFor={`tag-upload-${b.id}`}>
                          <Button type="button" variant="outline" size="sm" asChild disabled={uploadTagPhoto.isPending}>
                            <span>
                              ...
                              {uploadTagPhoto.isPending ? "인식 중..." : "태그 사진 촬영/업로드"}
                            </span>
                          </Button>
                        </label>
                      </div>
                    ...
                    {b.notes && (
                      <p className="text-xs text-muted-foreground bg-secondary/50 rounded p-2">{b.notes}</p>
                    )}
                    {b.statusUpdatedAt && (
                      <p className="text-[10px] text-muted-foreground">
                        마지막 업데이트: {new Date(b.statusUpdatedAt).toLocaleString("ko-KR")}
                      </p>
                    )}
                  </CardContent>
                </Card>
            ...
            {(!checkins || checkins.length === 0) ? (
              <Card className="bg-card border-border">
                <CardContent className="py-12 text-center">
                  ...
                  <p className="text-muted-foreground">체크인 정보가 없습니다.</p>
                  <p className="text-xs text-muted-foreground mt-1">관리자가 체크인 정보를 등록하면 여기에 표시됩니다.</p>
                </CardContent>
              </Card>
            ) : checkins.map((c: any) => {
              ...
              return (
                <Card key={c.id} className="bg-card border-border">
                  <CardContent className="p-4 space-y-3">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="font-bold text-lg">{c.flightNo || "항공편"}</p>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {c.checkinCounter && (
                        <div className="bg-secondary/50 rounded-lg p-3">
                          <p className="text-[10px] text-muted-foreground mb-1">체크인 카운터</p>
                      ...
                      {c.gateNumber && (
                        <div className="bg-secondary/50 rounded-lg p-3">
                          <p className="text-[10px] text-muted-foreground mb-1">탑승 게이트</p>
                          <p className="font-semibold flex items-center gap-1">
                      ...
                      {c.seatNumber && (
                        <div className="bg-secondary/50 rounded-lg p-3">
                          <p className="text-[10px] text-muted-foreground mb-1">좌석 번호</p>
                          <p className="font-semibold flex items-center gap-1">
                      ...
                      {c.boardingTime && (
                        <div className="bg-secondary/50 rounded-lg p-3">
                          <p className="text-[10px] text-muted-foreground mb-1">탑승 시간</p>
                          <p className="font-semibold flex items-center gap-1">
                            <Clock className="h-3.5 w-3.5 text-primary" />
                            {new Date(c.boardingTime).toLocaleTimeString("ko-KR", { hour: "2-digit", minute: "2-digit" })}
                          </p>
                        </div>
                      )}
                    </div>
                    ...
                    {c.boardingPassUrl && (
                      <div>
                        ...
                        <img src={c.boardingPassUrl} alt="탑승권" className="w-full rounded-lg border border-border" />
                      </div>
                    )}
