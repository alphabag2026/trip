...
export default function FlightTickets() {
  ...
  const createMutation = trpc.flightTicket.create.useMutation({
    onSuccess: () => { utils.flightTicket.listAll.invalidate(); setCreateOpen(false); resetForm(); toast.success(t("admin.flightTickets.t56", "항공권이 생성되었습니다")); },
    onError: (e) => toast.error(e.message),
  });
  const deleteMutation = trpc.flightTicket.delete.useMutation({
    onSuccess: () => { utils.flightTicket.listAll.invalidate(); toast.success(t("admin.flightTickets.t57", "삭제되었습니다")); },
    onError: (e) => toast.error(e.message),
  });
  ...
  const handleAutoGenerate = () => {
    ...
    toast.success(t("admin.flightTickets.t58", "예약번호와 티켓번호가 자동 생성되었습니다"));
  };

  const handleCreate = () => {
    if (!form.passengerName) {
      toast.error(t("admin.flightTickets.t59", "승객 이름은 필수입니다"));
      return;
    }
  ...
  if (isLoading) return <div className="p-6 text-center text-muted-foreground">{t("admin.flightTickets.t1", "로딩 중...")}</div>;

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold">{t("admin.flightTickets.t2", "항공권 관리")}</h1>
          <p className="text-muted-foreground">{t("admin.flightTickets.t3", "왕복 항공권을 등록하거나 임의 티켓을 생성합니다 (이미그레이션 통과용)")}</p>
        </div>
        <div className="flex gap-2">
          ...
          <Dialog open={createOpen} onOpenChange={setCreateOpen}>
            <DialogTrigger asChild>
              <Button><Plus className="w-4 h-4 mr-2" />{t("admin.flightTickets.t4", "항공권 생성")}</Button>
            </DialogTrigger>
            <DialogContent className="max-w-3xl max-h-[85vh] overflow-y-auto">
              <DialogHeader>
                <DialogTitle>{t("admin.flightTickets.t5", "항공권 생성")}</DialogTitle>
              </DialogHeader>
              <div className="grid gap-4">
                {/* 승객 정보 */}
                <div className="grid grid-cols-3 gap-3">
                  <div>
                    <Label>{t("admin.flightTickets.t6", "승객 이름 (영문) *")}</Label>
                    <Input value={form.passengerName} onChange={e => setForm(f => ({ ...f, passengerName: e.target.value }))} placeholder="HONG GILDONG" />
                  </div>
                  <div>
                    <Label>{t("admin.flightTickets.t7", "여권번호")}</Label>
                    <Input value={form.passportNumber} onChange={e => setForm(f => ({ ...f, passportNumber: e.target.value }))} placeholder="M12345678" />
                  </div>
                  <div>
                    <Label>{t("admin.flightTickets.t8", "국적")}</Label>
                ...
                <div className="flex gap-2 items-end">
                  ...
                  <Button variant="outline" onClick={handleAutoGenerate} type="button">
                    <Wand2 className="w-4 h-4 mr-1" />{t("admin.flightTickets.t11", "자동생성")}
                  </Button>
                </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        <Card>
          <CardContent className="pt-6 text-center">
            ...
            <div className="text-sm text-muted-foreground">{t("admin.flightTickets.t34", "전체 항공권")}</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-6 text-center">
            <div className="text-3xl font-bold text-blue-600">{tickets?.filter(t => t.isGenerated).length ?? 0}</div>
            <div className="text-sm text-muted-foreground">{t("admin.flightTickets.t35", "자동 생성 (입국용)")}</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-6 text-center">
            <div className="text-3xl font-bold text-green-600">{tickets?.filter(t => t.status === "active").length ?? 0}</div>
            <div className="text-sm text-muted-foreground">{t("admin.flightTickets.t36", "활성")}</div>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid gap-4">
        {(!tickets || tickets.length === 0) ? (
          <Card><CardContent className="py-12 text-center text-muted-foreground">{t("admin.flightTickets.t37", "아직 등록된 항공권이 없습니다")}</CardContent></Card>
        ) : tickets.map(tk => (
          <Card key={tk.id} className="hover:shadow-md transition-shadow">
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    {tk.isGenerated && <Badge variant="outline" className="text-orange-600 border-orange-600">{t("admin.flightTickets.t38", "자동생성")}</Badge>}
                  </div>
                  {/* 출발편 */}
                  {tk.outboundFlightNo && (
                ...
                <div className="flex gap-2">
                  <Button variant="outline" size="sm" onClick={() => setViewTicket(tk)}>
                    <Eye className="w-4 h-4" />
                  </Button>
                  <Button variant="outline" size="sm" className="text-red-600" onClick={() => {
                    if (confirm("삭제하시겠습니까?")) deleteMutation.mutate({ id: tk.id });
                  }}>
      ...
      <Dialog open={!!viewTicket} onOpenChange={() => setViewTicket(null)}>
        <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{t("admin.flightTickets.t39", "E-Ticket / 전자 항공권")}</DialogTitle>
          </DialogHeader>
          {viewTicket && (
            <div className="bg-white text-black p-6 rounded-lg border space-y-4">
              <div className="border-b pb-3 flex justify-between items-start">
                <div>
                  ...
                  <p className="text-sm text-gray-500">{t("admin.flightTickets.t40", "전자 항공권 영수증")}</p>
                </div>
                {viewTicket.isGenerated && (
                  <Badge className="bg-orange-100 text-orange-700">{t("admin.flightTickets.t41", "입국 편의용")}</Badge>
                )}
              </div>
              <div className="grid grid-cols-3 gap-4 text-sm">
                <div>
                  <p className="text-xs text-gray-500">{t("admin.flightTickets.t42", "Passenger Name / 승객명")}</p>
                  <p className="font-bold">{viewTicket.passengerName}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500">{t("admin.flightTickets.t43", "Booking Reference / 예약번호")}</p>
                  <p className="font-bold">{viewTicket.bookingReference || "-"}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500">{t("admin.flightTickets.t44", "Ticket Number / 티켓번호")}</p>
              ...
              {viewTicket.outboundFlightNo && (
                <div className="border rounded-lg p-4">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="font-semibold">{t("admin.flightTickets.t45", "Outbound Flight / 출발편")}</span>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <p className="text-xs text-gray-500">{t("admin.flightTickets.t46", "Flight / 편명")}</p>
                      <p className="font-bold">{viewTicket.outboundAirline} {viewTicket.outboundFlightNo}</p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-500">{t("admin.flightTickets.t47", "Class / 좌석등급")}</p>
                      <p>{viewTicket.outboundSeatClass || "Economy"}</p>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-4 mt-3">
                    <div className="border-l-4 border-blue-500 pl-3">
                      <p className="text-xs text-gray-500">{t("admin.flightTickets.t48", "Departure / 출발")}</p>
                    ...
                    <div className="border-l-4 border-green-500 pl-3">
                      <p className="text-xs text-gray-500">{t("admin.flightTickets.t49", "Arrival / 도착")}</p>
              ...
              {viewTicket.returnFlightNo && (
                <div className="border rounded-lg p-4">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="font-semibold">{t("admin.flightTickets.t50", "Return Flight / 귀국편")}</span>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <p className="text-xs text-gray-500">{t("admin.flightTickets.t51", "Flight / 편명")}</p>
                      <p className="font-bold">{viewTicket.returnAirline} {viewTicket.returnFlightNo}</p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-500">{t("admin.flightTickets.t52", "Class / 좌석등급")}</p>
                      <p>{viewTicket.returnSeatClass || "Economy"}</p>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-4 mt-3">
                    <div className="border-l-4 border-blue-500 pl-3">
                      <p className="text-xs text-gray-500">{t("admin.flightTickets.t53", "Departure / 출발")}</p>
                    ...
                    <div className="border-l-4 border-green-500 pl-3">
                      <p className="text-xs text-gray-500">{t("admin.flightTickets.t54", "Arrival / 도착")}</p>
              ...
              <div className="text-xs text-gray-400 border-t pt-3">
                {t("admin.flightTickets.t55", "This is an electronic ticket. No paper ticket will be issued. / 전자 항공권입니다. 종이 티켓은 발행되지 않습니다.")}
              </div>
            </div>
          )}
        </DialogContent>
