...
export default function AdminExpenses() {
  ...
  const createMut = trpc.expense.create.useMutation({
    onSuccess: () => { toast.success("비용 등록 완료"); setShowAddDialog(false); refetch(); refetchAll(); resetForm(); },
    onError: (e) => toast.error(e.message),
  });
  const updateMut = trpc.expense.update.useMutation({
    onSuccess: () => { toast.success("비용 수정 완료"); setEditingExpense(null); refetch(); refetchAll(); resetForm(); },
    onError: (e) => toast.error(e.message),
  });
  const deleteMut = trpc.expense.delete.useMutation({
    onSuccess: () => { toast.success("비용 삭제 완료"); refetch(); refetchAll(); },
    onError: (e) => toast.error(e.message),
  });
  ...
  const handleAdd = () => {
    if (!form.meetupId) return toast.error("밋업을 선택해주세요");
    if (!form.title) return toast.error("항목명을 입력해주세요");
    if (form.amount <= 0) return toast.error("금액을 입력해주세요");
  ...
  const ExpenseForm = ({ onSubmit, submitLabel }: { onSubmit: () => void; submitLabel: string }) => (
    <div className="space-y-4">
      ...
      <div>
        <Label>항목명 *</Label>
        <Input value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} placeholder="예: 방콕행 항공권" />
      </div>
      <div>
        <Label>설명</Label>
        <Textarea value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} placeholder="상세 내용" rows={2} />
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div>
          <Label>금액 *</Label>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <Label>지출자</Label>
          <Input value={form.paidBy} onChange={e => setForm(f => ({ ...f, paidBy: e.target.value }))} placeholder="주최측 담당자명" />
        </div>
        <div>
          <Label>대상 (팀/개인)</Label>
          <Input value={form.paidFor} onChange={e => setForm(f => ({ ...f, paidFor: e.target.value }))} placeholder="팀A, 홍길동" />
        </div>
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div>
        ...
        <div>
          ...
          <Select value={form.registeredVia} onValueChange={v => setForm(f => ({ ...f, registeredVia: v }))}>
            ...
            <SelectContent>
              ...
              <SelectItem value="qr_scan">QR 스캔</SelectItem>
            </SelectContent>
          </Select>
        </div>
      </div>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex gap-2">
          ...
          <Button onClick={() => { resetForm(); setShowAddDialog(true); }} size="sm">
            <Plus className="w-4 h-4 mr-2" /> 비용 등록
          </Button>
        </div>
      </div>
          ...
          <Card>
            <CardContent className="p-4 flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-sm text-muted-foreground">참가자</div>
              </div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4 flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-sm text-muted-foreground">1인당 비용</div>
              </div>
            </CardContent>
          </Card>
        </div>
      ...
      {summary && selectedMeetupId && summary.byCategory.length > 0 && (
        <Card>
          <CardHeader><CardTitle className="text-base">카테고리별 지출</CardTitle></CardHeader>
          <CardContent>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b bg-muted/50">
                  ...
                  <th className="text-center p-3 font-medium">경로</th>
              ...
              {filtered.length > 0 && (
                <tfoot>
                  <tr className="border-t-2 bg-muted/30">
                    <td colSpan={4} className="p-3 font-semibold text-right">합계</td>
