...
export default function Channels() {
  ...
  const createMutation = trpc.channel.create.useMutation({
    onSuccess: () => { toast.success("채널이 생성되었습니다"); setShowCreate(false); refetch(); resetForm(); },
    onError: (e) => toast.error(e.message),
  });
  const deleteMutation = trpc.channel.delete.useMutation({
    onSuccess: () => { toast.success("채널이 삭제되었습니다"); refetch(); },
    onError: (e) => toast.error(e.message),
  });
  ...
  const handleCreate = () => {
    if (!form.channelName.trim()) { toast.error("채널명을 입력하세요"); return; }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold">소통 채널 관리</h1>
          <p className="text-muted-foreground text-sm">픽업 기사, 매니저, 호텔 체크인 담당자와의 소통 채널</p>
        </div>
        <Dialog open={showCreate} onOpenChange={setShowCreate}>
          <DialogTrigger asChild>
            <Button><Plus className="mr-2 h-4 w-4" /> 채널 생성</Button>
          </DialogTrigger>
          <DialogContent>
            ...
            <div className="space-y-4">
              ...
              <div>
                <Label>담당자 이름</Label>
                <Input value={form.assignedTo} onChange={(e) => setForm({ ...form, assignedTo: e.target.value })} placeholder="담당자 이름" />
              </div>
              <div>
              ...
              <div>
                <Label>설명</Label>
                <Textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="채널 설명" />
              </div>
              <Button className="w-full" onClick={handleCreate} disabled={createMutation.isPending}>생성</Button>
            </div>
          </DialogContent>
        </Dialog>
      </div>
            ...
            <CardContent className="space-y-3">
              ...
              <div className="flex gap-2">
                <Button variant="outline" size="sm" className="flex-1" onClick={() => copyLink(ch.id)}>
                  <Copy className="mr-1 h-3 w-3" /> 링크 복사
                </Button>
