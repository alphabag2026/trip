...
export default function AdminBaggageCheckin() {
  ...
  const updateBaggageStatus = trpc.baggage.updateStatus.useMutation({
    onSuccess: () => { refetchBaggage(); toast.success("수화물 상태가 업데이트되었습니다."); },
  });
  const deleteBaggage = trpc.baggage.delete.useMutation({
    onSuccess: () => { refetchBaggage(); toast.success("수화물이 삭제되었습니다."); },
  });
  const createCheckin = trpc.checkin.create.useMutation({
    onSuccess: () => { refetchCheckins(); setShowAddCheckin(false); toast.success("체크인 정보가 등록되었습니다."); },
  });
  const updateCheckin = trpc.checkin.update.useMutation({
    onSuccess: () => { refetchCheckins(); toast.success("체크인 정보가 업데이트되었습니다."); },
  });
  const deleteCheckin = trpc.checkin.delete.useMutation({
    onSuccess: () => { refetchCheckins(); toast.success("체크인 정보가 삭제되었습니다."); },
  });
  ...
  const handleAddCheckin = (e: React.FormEvent) => {
    e.preventDefault();
    if (!checkinForm.registrationId) { toast.error("참석자를 선택해주세요."); return; }
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            <Luggage className="h-6 w-6 text-primary" />수화물 & 체크인 관리
          </h1>
          <p className="text-sm text-muted-foreground mt-1">수화물 추적 상태 및 체크인 정보를 관리합니다</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card className="bg-card border-border">
          <CardContent className="p-4 text-center">
            ...
            <p className="text-xs text-muted-foreground">등록 수화물</p>
          </CardContent>
        </Card>
        <Card className="bg-card border-border">
          <CardContent className="p-4 text-center">
        ...
        <Card className="bg-card border-border">
          <CardContent className="p-4 text-center">
            ...
            <p className="text-xs text-muted-foreground">체크인 등록</p>
          </CardContent>
        </Card>
        <Card className="bg-card border-border">
          <CardContent className="p-4 text-center">
            ...
            <p className="text-xs text-muted-foreground">탑승 완료</p>
          </CardContent>
        </Card>
      </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-border text-left">
                      <th className="p-3 font-medium">참석자</th>
                      <th className="p-3 font-medium">태그번호</th>
                      <th className="p-3 font-medium">종류</th>
                      <th className="p-3 font-medium">무게</th>
                      <th className="p-3 font-medium">상태</th>
                      <th className="p-3 font-medium">태그사진</th>
                      <th className="p-3 font-medium">작업</th>
                    </tr>
                  </thead>
                  <tbody>
                    {(!filteredBaggage || filteredBaggage.length === 0) ? (
                      <tr><td colSpan={7} className="p-8 text-center text-muted-foreground">등록된 수화물이 없습니다.</td></tr>
                          ...
                          <td className="p-3">
                            {b.tagPhotoUrl ? (
                              <Dialog>
                                ...
                                <DialogContent>
                                  <DialogHeader><DialogTitle>수화물 태그 사진</DialogTitle></DialogHeader>
                                  <img src={b.tagPhotoUrl} alt="태그" className="w-full rounded-lg" />
                                  {b.ocrResult && (
                                    <div className="bg-secondary rounded-lg p-3 text-sm space-y-1">
                                      <p><strong>태그번호:</strong> {b.ocrResult.tagNumber}</p>
                                      <p><strong>항공사:</strong> {b.ocrResult.airline}</p>
                                      <p><strong>목적지:</strong> {b.ocrResult.destination}</p>
                                      <p><strong>신뢰도:</strong> {b.ocrResult.confidence}</p>
                                    </div>
                                  )}
                                </DialogContent>
                              </Dialog>
        ...
        <TabsContent value="checkin" className="space-y-4">
          <div className="flex justify-end">
            <Dialog open={showAddCheckin} onOpenChange={setShowAddCheckin}>
              <DialogTrigger asChild>
                <Button size="sm"><Plus className="h-4 w-4 mr-1" />체크인 정보 등록</Button>
              </DialogTrigger>
              <DialogContent className="max-w-lg">
                <DialogHeader><DialogTitle>체크인 정보 등록</DialogTitle></DialogHeader>
                <form onSubmit={handleAddCheckin} className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <Label>항공사</Label>
                      <Input value={checkinForm.airline} onChange={e => setCheckinForm(p => ({ ...p, airline: e.target.value }))} placeholder="대한항공" />
                    </div>
                    <div>
                      <Label>편명</Label>
                  ...
                  <div>
                    <Label>메모</Label>
                    <Textarea value={checkinForm.notes} onChange={e => setCheckinForm(p => ({ ...p, notes: e.target.value }))} placeholder="특이사항" />
                  </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-border text-left">
                      <th className="p-3 font-medium">참석자</th>
                      <th className="p-3 font-medium">항공편</th>
                      <th className="p-3 font-medium">카운터</th>
                      <th className="p-3 font-medium">게이트</th>
                      <th className="p-3 font-medium">좌석</th>
                      <th className="p-3 font-medium">탑승시간</th>
                      <th className="p-3 font-medium">상태</th>
                      <th className="p-3 font-medium">작업</th>
                    </tr>
                  </thead>
                  <tbody>
                    {(!filteredCheckins || filteredCheckins.length === 0) ? (
                      <tr><td colSpan={8} className="p-8 text-center text-muted-foreground">등록된 체크인 정보가 없습니다.</td></tr>
