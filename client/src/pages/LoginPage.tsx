          ...
          <Card className="border-slate-700 bg-slate-800/50 backdrop-blur-sm shadow-2xl">
            ...
            <CardContent>
              ...
              <button
                ...
                onClick={() => {
                  const origin = window.location.origin;
                  window.location.href = `/api/auth/google?origin=${encodeURIComponent(origin)}&returnPath=${encodeURIComponent(returnPath)}`;
                }}
                className="w-full flex items-center justify-center gap-2 py-2.5 px-4 rounded-lg font-medium text-sm transition-all border border-slate-600 bg-white hover:bg-gray-50 mt-2"
              >
                <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
                ...
                <span style={{ color: '#333' }}>{t("loginPage.googleLogin", "Google로 로그인")}</span>
              </button>
